...
struct traits<char[Size]> : traits<char*> {};
...
struct traits<Type> : traits<std::underlying_type_t<Type>> {
  static auto fwd_args(Type const& e) { return std::tuple{static_cast<std::underlying_type_t<Type>>(e)}; }
};
...
struct traits<Str> {
  static auto fwd_args(Str const& arg) { return std::tuple{arg.c_str()}; }
...
template <concepts::container_of_char Str>
  requires(
      !requires(Str s) { s.c_str(); }  // these are handled separately
      &&
      requires(Str s) {
        std::data(s);
        std::size(s);
...
struct traits<Str> {
  static auto fwd_args(Str const& arg) { return std::tuple{static_cast<int>(std::size(arg)), std::data(arg)}; }
  static constexpr auto spec = ".*s";
...
class transformer {
 ...
 private:
  ...
  constexpr status transform_priv(char const*& src) noexcept {
    constexpr specifier specifiers[] = {specifier{traits<Args>::spec, flags<Args>()}..., specifier{}};
  ...
  static constexpr bool at_end(char const* const ptr) noexcept { return *ptr == '\0'; }

  // Printf format string: %[flags][width][.precision][length]specifier
  constexpr status find_specifier(char const*& src, specifier const*) noexcept;
  constexpr status transform_specifier(char const*& src, specifier const*) noexcept;
};

// The job of this function is to copy text verbatim until it finds a format
// specifier.
constexpr status transformer::find_specifier(char const*& src, specifier const* spec_array) noexcept {
  while (!at_end(src)) {
    if (*src == '%') {
      append('%');
      if (at_end(++src)) return status::format_spurious_percent;
...
constexpr status transformer::transform_specifier(char const*& src, specifier const* spec_array) noexcept {
  ...
  for (auto&& field : {status::field_width_needs_int, status::field_precision_needs_int}) {
    if (at_end(src)) return status::conversion_lacks_type;

    if (field == status::field_precision_needs_int) {  // require dot first
      if (*src == '.') {
        if (spec_array->flags & forbid_precision) return status::field_precision_not_allowed;
        append('.');
        if (at_end(++src)) return status::conversion_lacks_type;
      } else {
  ...
  for (int i = 1; i <= 4; ++i) {  // this could not be more than 4 chars
    ...
    if (ch == '?') {
      ...
      if (ch == 'c') {  // %c takes no sub-specifiers
        if (!(spec_array->flags & promotes_to_int)) return status::format_expects_char;
      } else if (ch == 'n') {  // %n takes no sub-specifiers
        if (!(spec_array->flags & record_position)) return status::format_expects_int_ptr;
      } else if (ch == 'p') {  // %p takes no sub-specifiers
        if (!(spec_array->flags & prints_as_pointer)) return status::format_expects_ptr;
      } else {
...
class appending_transformer : public transformer {
 public:
  constexpr appending_transformer(char* out_) : out{out_} {}
...
decltype(auto) invoke(Function const& call, Args const&... args) {
  if constexpr (sizeof...(args) == 0)
    ...
    return std::apply(call, std::tuple_cat(detail::fwd_args(args)...));
}
...
[[gnu::always_inline, gnu::flatten]] inline int printf(Args const&... args) noexcept {
  return printx::invoke(
      [](auto const&... args_) {
        static constexpr auto fmt = printx::build_fmt<Fmt, Args...>();
        return std::printf(fmt.data, args_...);
      },
      args...);
}

template <printx::literal Fmt, typename Stream, typename... Args>
[[gnu::always_inline, gnu::flatten]] inline int fprintf(Stream const& stream, Args const&... args) noexcept {
  return printx::invoke(
      [&](auto const&... args_) {
        static constexpr auto fmt = printx::build_fmt<Fmt, Args...>();
        return std::fprintf(stream, fmt.data, args_...);
      },
      args...);
}

template <printx::literal Fmt, typename... Args>
[[gnu::always_inline, gnu::flatten]] inline int snprintf(char* s, std::size_t n, Args const&... args) noexcept {
  return printx::invoke(
      [&](auto const&... args_) {
        static constexpr auto fmt = printx::build_fmt<Fmt, Args...>();
        return std::snprintf(s, n, fmt.data, args_...);
      },
      args...);
}

template <printx::literal Fmt, typename Buffer, typename... Args>
  requires requires(Buffer b) {
    std::data(b);
    std::size(b);
  }
[[gnu::always_inline, gnu::flatten]] inline int sprintf(Buffer&& buffer, Args const&... args) noexcept {
  return printx::invoke(
      [&](auto const&... args_) {
        static constexpr auto fmt = printx::build_fmt<Fmt, Args...>();
        return std::snprintf(std::data(buffer), std::size(buffer), fmt.data, args_...);
      },
      args...);
}
