...
static T ThrowIfNotValid(const common::Args& cli, std::string_view key) {
  if (!cli.Has(key)) {
    const char* error_msg = common::FormatIntoCString<"Input flag %s is not specified\n">(key);
    throw std::runtime_error(error_msg);
  }
  if constexpr (std::is_same_v<T, std::string_view>) {
    std::optional<std::string_view> value = cli.Value({key});
    if (!value.has_value()) {
      const char* error_msg = common::FormatIntoCString<"Input value to flag %s is not specified\n">(key);
      throw std::runtime_error(error_msg);
    }
    return value.value();
